...
class Mute(commands.Cog):
    ...
    async def mute_member_text(self, interaction: Interaction, member, days, hours, minutes, seconds, reason):
        ...
        if muted in member.roles:
            await interaction.response.send_message(f"<@{member.id}> has been **already muted**!", ephemeral=True)
            return
        else:
            if reason == None:
                await member.add_roles(muted)
                await interaction.response.send_message(f":white_check_mark: <@{member.id}> has been **muted from text** for **{days}** days, **{hours}** hours, **{minutes}** minutes, and **{seconds}** seconds. :zipper_mouth:")
            else:
                await member.add_roles(muted, reason=reason)
                await interaction.response.send_message(f":white_check_mark: <@{member.id}> has been **muted from text** for **{days}** days, **{hours}** hours, **{minutes}** minutes, and **{seconds}** seconds. :zipper_mouth: Reason: **{reason}**")
    ...
    async def mute_member_voice(self, interaction: Interaction, member, days, hours, minutes, seconds, reason):
        ...
        if muted in member.roles:
            ...
            if reason == None:
                ...
                await interaction.response.send_message(f":white_check_mark: <@{member.id}> has been **muted from voice** for **{days}** days, **{hours}** hours, **{minutes}** minutes, and **{seconds}** seconds. :zipper_mouth:")
            else:
                await member.edit(mute=True, reason=reason)
                await interaction.response.send_message(f":white_check_mark: <@{member.id}> has been **muted from voice** for **{days}** days, **{hours}** hours, **{minutes}** minutes, and **{seconds}** seconds. :zipper_mouth: Reason: **{reason}**")
            await asyncio.sleep(duration.total_seconds())
    ...
    async def mute_voice(self, interaction: Interaction, member: discord.Member, reason: Optional[str] = None, days: Optional[app_commands.Range[int, 0]] = 0, hours: Optional[app_commands.Range[int, None, 23]] = 0, minutes: Optional[app_commands.Range[int, None, 59]] = 0, seconds: Optional[app_commands.Range[int, None, 59]] = 0):  # setting each value with a default value of 0 reduces a lot of the code
        if member.id == interaction.user.id:
            await interaction.response.send_message("BRUH! You can't **mute yourself from voice**!")
            return
        elif member.id == self.bot.application_id:
            # To prevent the bot bans itself from the server by accident
            await interaction.response.send_message(f"i cannot just **mute myself from voice** ^u^")
        elif member.guild_permissions.administrator:
            if interaction.user.id == interaction.guild.owner.id:
                await self.mute_member_text(interaction, member, days, hours, minutes, seconds, reason)
            else:
