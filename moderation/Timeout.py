...
class Timeout(commands.Cog):
    ...
    async def timeout_member(self, interaction: Interaction, member, days, hours, minutes, seconds, reason):
        ...
        if reason == None:
            ...
            await interaction.response.send_message(f"<@{member.id}> has been **timed out** for **{days}** days, **{hours}** hours, **{minutes}** minutes, and **{seconds}** seconds.")
        else:
            await member.timeout(duration, reason=reason)
            await interaction.response.send_message(f"<@{member.id}> has been **timed out** for **{days}** days, **{hours}** hours, **{minutes}** minutes, and **{seconds}** seconds by for **'{reason}'**.")
